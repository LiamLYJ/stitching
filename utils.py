...
def importData(fileName, imageDirectory, workspaceDir):
    '''
    ...
    :return: dataMatrix: A NumPy ndArray contaning all of the pose data. Each row stores 6 floats containing pose information in XYZYPR form
        allImages: A Python List of images filename.
    '''
    ...
    for i in range(0,fileNameMatrix.shape[0]): #read images
        ...
        allImages.append(os.path.join(imageDirectory, fileNameMatrix[i]))
        # cv2.imwrite(os.path.join(workspaceDir, str(i).zfill(4) + ".png"), cv2.imread(os.path.join(imageDirectory,fileNameMatrix[i])))
