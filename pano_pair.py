...
def combine(image1, image2, detector, valid_ratio = 0.2, display = False):
    ...
    good = []
    for m, n in matches:
        if m.distance < valid_ratio * n.distance:
            good.append(m)
    ...
    matchDrawing = utils.drawMatches(gray2,kp2,gray1,kp1,matches)
    if display:
        utils.display("matches",matchDrawing)
    ...
    #utils.display("r", warpedResImg, 400000)
